...
@ApiTags('')
@Controller('income-investment-requests')
export class InvestmentRequestController {
  ...
  async getIncomeValuation(@Res() res: Response): Promise<any> {
    try {
      const valuation =
        await this.investmentRequestIncomeService.getIncomeValuation();
  ...
  async createInvestmentRequest(
    ...
    if (createInvestmentRequestDto.price < 100) {
      throw new BadRequestException('Minimum investment amount is 100');
    }

    try {
      const result =
        await this.investmentRequestIncomeService.createInvestmentRequest(
          user_id,
          createInvestmentRequestDto,
        );
  ...
  async getUserInvestmentRequests(
    ...
    try {
      const result =
        await this.investmentRequestIncomeService.getUserInvestmentRequests(
          user_id,
          queryParams,
        );
  ...
  async getUserFundAccounts(@Req() request: Request, @Res() res: Response) {
    ...
    try {
      const result =
        await this.investmentRequestIncomeService.getUserFundAccounts(user_id);
  ...
  async getCustomerFundAccounts(
    ...
    try {
      const result =
        await this.investmentRequestIncomeService.getCustomerFundAccounts(
          customerId,
        );
  ...
  async getUserRedemptionRequests(
    ...
    const user_id_raw = request['whoAmmI']?.user_id;
    console.log('user_id_raw', user_id_raw);
    if (!user_id_raw || isNaN(Number(user_id_raw))) {
    ...
    try {
      const result =
        await this.investmentRequestIncomeService.getUserRedemptionRequests(
          user_id,
          queryParams,
        );
  ...
  async updateInvestmentRequest(
    ...
    try {
      const result =
        await this.investmentRequestIncomeService.updateInvestmentRequest(
          user_id,
          requestId,
          updateInvestmentRequestDto,
        );
  ...
  async createFundRedemption(
    ...
    try {
      const result = await this.investmentRequestIncomeService.fundRedemption(
        user_id,
        createFundRedemptionDto,
      );
  ...
  async getUserTotalActiveInvestment(
    ...
    try {
      const total =
        await this.investmentRequestIncomeService.getUserTotalActiveInvestmentAmount(
          user_id,
        );
      this.logger.log(
