...
async function init() {
  try {
    console.log('--- Initializing Full Admin Setup v2 ---');
    ...
    await runQuery(
      ...
      INSERT INTO admin (
        ...
        activationStatus, user_type, account_status, 
    ...
    const admin = await runQuery(
      'SELECT staffId FROM admin WHERE staffEmail = ? LIMIT 1',
      [email],
    );
    const staffId = admin[0].staffId;

    // 6. Linking Tables (Handle missing tables gracefully)
    console.log('Ensuring Junction Tables exist and linking...');
    ...
    await runQuery(
      `
      INSERT IGNORE INTO admin_roles_admin_role (adminStaffId, adminRoleId) 
      VALUES (?, ?)
    `,
      [staffId, roleId],
    );

    // C. Direct role link (if exist)
    await runQuery('UPDATE admin SET roles_id = ? WHERE staffId = ?', [
      roleId,
      staffId,
    ]).catch(() => {
      console.log(
        'Warning: Direct roles_id update failed (column might not exist).',
      );
    });
