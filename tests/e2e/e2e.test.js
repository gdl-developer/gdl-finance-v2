...
async function runTest() {
  ...
  try {
    ...
    let encryptedData = cipher.update(JSON.stringify(loginData), 'utf8', 'base64');
    ...
    console.log(`Public Key used (first 50 chars): ${publicKey.substring(0, 50)}`);

    const payload = {
      is_encrypted: true,
      key: encryptedKey.toString('base64'),
      iv: iv.toString('base64'),
      payload: Buffer.concat([Buffer.from(encryptedData, 'base64'), tag]).toString('base64')
    };
    ...
    try {
      ...
      console.log('❌ Plain Login Failed:', e.response ? e.response.data : e.message);
    }
    ...
    if (loginRes.data.success || loginRes.data.access_token) {
      console.log('✅ Login Successful (or correctly rejected if user non-existent)');
    } else {
